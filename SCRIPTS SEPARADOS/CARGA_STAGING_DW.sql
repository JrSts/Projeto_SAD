...
WHILE @@FETCH_STATUS = 0
	BEGIN
		...
		ELSE
			BEGIN
				IF(EXISTS(SELECT COD_DIVIDA FROM DW.FATO_DIVIDA WHERE COD_DIVIDA = @COD_DIVIDA))
					UPDATE DW.DIM_DIVIDA
					SET CODIGO = @COD_DIVIDA, COD_CLIENTE = @COD_CLIENTE, COD_FUNCIONARIO = @COD_FUNCIONARIO,
					    COD_CATEGORIA = @COD_CATEGORIA, DATA = @DATA_DIVIDA, VALOR = @VALOR
					WHERE COD_DIVIDA = @COD_DIVIDA
				ELSE
					BEGIN
						...
						@ID_DATA_POSSIVEL_PAGAMENTO, @VALOR_PAGAMENTO, 1)
...
DECLARE @DATA DATETIME, @COD_PAGAMENTO INT, @COD_CLIENTE INT, @COD_FUNCIONARIO INT, @DATA_PAGAMENTO VARCHAR(50), @VALOR NUMERIC(10,2),
@ID_CLIENTE INT, @ID_FUNCIONARIO INT, @ID_DATA_PAGAMENTO DATETIME, @VALOR_PAGAMENTO NUMERIC(10,2)
...
OPEN C_CURSOR
FETCH C_CURSOR INTO @DATA, @COD_DIVIDA, @COD_CLIENTE, @COD_FUNCIONARIO, @DATA_DIVIDA, @VALOR
WHILE @@FETCH_STATUS = 0
	BEGIN
		...
		ELSE
			BEGIN
				...
				INSERT INTO DW.FATO_DIVIDA VALUES (@ID_CLIENTE, @ID_FUNCIONARIO, @ID_DATA_PAGAMENTO, 
				@VALOR_PAGAMENTO, 1)
			END
		FETCH C_CURSOR INTO @DATA, @COD_DIVIDA, @COD_CLIENTE, @COD_FUNCIONARIO, @DATA_DIVIDA, @VALOR
	END
